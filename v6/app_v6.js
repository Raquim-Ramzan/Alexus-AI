...
function renderMessages() {
    ...
    currentSession.messages.forEach((msg, index) => {
        ...
        // Display uploaded or screen-captured images
        if (msg.imageData) {
...
// Image download handler removed (Image generation features disabled)
...
async function uploadFile() {
    try {
        ...
        if (result.success) {
            ...
            if (result.isImage) {
                addMessage('user', `📎 Attached: ${result.fileName}`, result.fileData);
            } else {
...
window.addEventListener('load', () => {
    ...
    if (!currentSession || currentSession.messages.length === 0) {
        addMessage('system', '⚡ ALEXUS V6 initialized. Enhanced with file upload & screen analysis.');
    }
});
