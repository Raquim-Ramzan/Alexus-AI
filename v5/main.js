...
if (!GEMINI_API_KEY) {
    console.error('⚠️  WARNING: GEMINI_API_KEY not found in API_Keys.env');
}
if (!GROQ_API_KEY) {
    console.error('⚠️  WARNING: GROQ_API_KEY not found in API_Keys.env');
}
...
function startPythonService() {
    ...
    const pythonScript = path.join(__dirname, 'voice_service_v5.py');
...
function createExpressServer() {
    ...
    expressApp.post('/api/query', async (req, res) => {
        try {
            ...
            if (query.toLowerCase().match(/generate|create|draw|make.*(?:image|picture|photo|art)/i)) {
                return await handleImageGeneration(query, res);
            }

            // Check if it's a system command - PRIORITY CHECK
...
async function handleSystemCommand(query) {
    ...
    // ===== FILE OPERATIONS =====
    if (lowerQuery.match(/(open|move|copy|rename|delete|create)\s+(file|folder)/i)) {
    ...
    // ===== SCREENSHOT =====
    if (lowerQuery.match(/take\s+(a\s+)?screenshot|capture\s+screen/i)) {
    ...
    // ===== RUN SCRIPTS =====
    if (lowerQuery.match(/run\s+(script|python|powershell|batch|cmd)/i)) {
        return await handleScriptExecution(lowerQuery);
    }
...
async function handleFileOperation(query) {
    // Basic file operations - would need specific paths
    if (query.match(/open\s+file\s+explorer|open\s+files/i)) {
        const command = platform === 'win32' ? 'explorer' : platform === 'darwin' ? 'open .' : 'nautilus';
        exec(command);
        return { handled: true, message: '📁 Opening File Explorer...' };
    }

    return { handled: true, message: '📁 For specific file operations, please provide the file path' };
}
...
async function handleScreenshot() {
    if (platform === 'win32') {
        exec('powershell -command "$obj = New-Object -ComObject WScript.Shell; $obj.SendKeys(\'{PRTSC}\')"');
        return { handled: true, message: '📸 Screenshot taken!' };
    }
    return { handled: false };
}
...
// SCRIPT EXECUTION
async function handleScriptExecution(query) {
    return { handled: true, message: '⚠️ Script execution requires file path. Use: "run script C:\\path\\to\\script.py"' };
}
...
ipcMain.handle('save-image', async (event, imageData) => {
    const result = await dialog.showSaveDialog(mainWindow, {
        defaultPath: `ALEXUS-generated-${Date.now()}.png`,
